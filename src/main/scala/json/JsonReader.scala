...
/** Parses a character-stream into a JSON value.
  * This is guaranteed to read no more than necessary to ensure it
  * has reached the end of the object -- for object, arrays, strings,
  ...
  * the end.
  *
  * This class does many small reads; it is a good idea to wrap
  * the input reader into a [[java.io.BufferedReader]]. */
class JsonReader(r: Reader) {
  ...
  private def readNumber(): JNumber = {
    ...
    //    -?\d+(\.\d+)?([eE][+-]?\d+)?
    // We'll match the whole thing, more or less
    val sb = new StringBuilder
