...
sealed trait ZipperLike[Parent] {
  ...
  def next(implicit ev: Parent <:< JCompoundZipper[_]): Option[JsonZipper[Parent]]
  def prev(implicit ev: Parent <:< JCompoundZipper[_]): Option[JsonZipper[Parent]]
  ...
  def position_?(implicit ev: Parent <:< JArrayZipper[_]): Option[Int]
  def field_?(implicit ev: Parent <:< JObjectZipper[_]): Option[String]
...
sealed trait NothingZipper[Parent] extends ZipperLike[Parent] {
  def top(implicit ev: Parent <:< JCompoundZipper[_]): JsonZipper[_]
}
...
sealed trait JsonZipper[Parent] extends ZipperLike[Parent] {
  ...
  def down(idx: Int)(implicit ev: Self <:< JArrayZipper[_]): JsonZipper[Self]
  def down(field: String)(implicit ev: Self <:< JObjectZipper[_]): JsonZipper[Self]

  def down_?(idx: Int): Option[JsonZipper[Self]]
  def down_?(field: String): Option[JsonZipper[Self]]

  def first(implicit ev: Self <:< JCompoundZipper[_]): JsonZipper[Self]
  def last(implicit ev: Self <:< JCompoundZipper[_]): JsonZipper[Self]

  def first_? : Option[JsonZipper[Self]]
  def last_? : Option[JsonZipper[Self]]

  def remove: NothingZipper[Parent]
  def remove(idx: Int)(implicit ev: Self <:< JArrayZipper[_]): Self = down(idx).remove.up
  def remove(field: String)(implicit ev: Self <:< JObjectZipper[_]): Self = down(field).remove.up
...
sealed trait JAtomZipper[Parent] extends JsonZipper[Parent] {
  ...
  def down(idx: Int)(implicit ev: Self <:< JArrayZipper[_]): JsonZipper[Self] = JsonZipper.badUserNoCookie()
  ...
  def first(implicit ev: Self <:< JCompoundZipper[_]): JsonZipper[Self] = JsonZipper.badUserNoCookie()
  def last(implicit ev: Self <:< JCompoundZipper[_]): JsonZipper[Self] = JsonZipper.badUserNoCookie()
...
sealed trait JArrayZipper[Parent] extends JCompoundZipper[Parent] {
  ...
  def first(implicit ev: Self <:< JCompoundZipper[_]): JsonZipper[Self] = down(0)
  def last(implicit ev: Self <:< JCompoundZipper[_]): JsonZipper[Self] = down(size - 1)
...
sealed trait JObjectZipper[Parent] extends JCompoundZipper[Parent] {
  ...
  def first(implicit ev: Self <:< JCompoundZipper[_]): JsonZipper[Self] = down(fields.first)
  def last(implicit ev: Self <:< JCompoundZipper[_]): JsonZipper[Self] = down(fields.last)
...
private[zipper] class TopLevelZipperLike { this: ZipperLike[Root] =>
  def up(implicit ev: Root <:< JCompoundZipper[_]) = JsonZipper.badUserNoCookie()
  def up_? = None

  def next(implicit ev: Root <:< JCompoundZipper[_]): Option[JsonZipper[Root]] = JsonZipper.badUserNoCookie()
  def prev(implicit ev: Root <:< JCompoundZipper[_]): Option[JsonZipper[Root]] = JsonZipper.badUserNoCookie()
  ...
  def position_?(implicit ev: Root <:< JArrayZipper[_]): Option[Int] = None
  def field_?(implicit ev: Root <:< JObjectZipper[_]): Option[String] = None
...
private[zipper] object TopLevelNothingZipper extends TopLevelZipperLike with NothingZipper[Root] {
  def top(implicit ev: Root <:< JCompoundZipper[_]) = JsonZipper.badUserNoCookie()
  def top_? = None
}
...
private[zipper] trait ElementNothingZipper[Parent <: JCompoundZipper[_]] { this: ElementZipperLike[Parent] with NothingZipper[Parent] =>
  def top(implicit ev: Parent <:< JCompoundZipper[_]): JsonZipper[_] = up.top
...
private[zipper] abstract class ArrayElementZipperLike[Parent <: JArrayZipper[_]](p: Parent, val idxInParent: Int) extends ElementZipperLike(p) { this: ZipperLike[Parent] =>
  ...
  def next(implicit ev: Parent <:< JCompoundZipper[_]): Option[JsonZipper[Parent]] = {
    val p = up
    val nextIdx = idxInParent + 1
    if(nextIdx < p.size) Some(p.down(nextIdx))
    None
  }

  def prev(implicit ev: Parent <:< JCompoundZipper[_]): Option[JsonZipper[Parent]] = {
    val p = up
    val prevIdx = idxInParent - 1
    if(prevIdx >= 0) Some(p.down(prevIdx))
    None
  }
  ...
  def position_?(implicit ev: Parent <:< JArrayZipper[_]): Option[Int] = Some(position)
  def field_?(implicit ev: Parent <:< JObjectZipper[_]): Option[String] = None
}
...
private[zipper] abstract class ObjectElementZipperLike[Parent <: JObjectZipper[_]](p: Parent, val fieldInParent: String) extends ElementZipperLike(p) { this: ZipperLike[Parent] =>
  ...
  def position_?(implicit ev: Parent <:< JArrayZipper[_]): Option[Int] = None
  def field_?(implicit ev: Parent <:< JObjectZipper[_]): Option[String] = Some(field)
}
...
private[zipper] abstract class ChangedArrayElementZipper[Parent <: JArrayZipper[_]](p: Parent, i: Int) extends ArrayElementZipper(p, i) { this: JsonZipper[Parent] =>
  override def up(implicit ev: Parent <:< JCompoundZipper[_]) = parent.replace(JArray(parent.here.toSeq.updated(idxInParent, here))).asInstanceOf[Parent]
}
...
private[zipper] class ChangedArrayElementNothingZipper[Parent <: JArrayZipper[_]](p: Parent, i: Int) extends ArrayElementZipperLike(p, i) with ElementNothingZipper[Parent] with NothingZipper[Parent] {
  override def up(implicit ev: Parent <:< JCompoundZipper[_]) = parent.replace(JArray(parent.here.toSeq.patch(idxInParent, Nil, 1))).asInstanceOf[Parent]
}
...
private[zipper] abstract class ChangedObjectElementZipper[Parent <: JObjectZipper[_]](p: Parent, f: String) extends ObjectElementZipper(p, f) with ElementZipper[Parent] { this: JsonZipper[Parent] =>
  override def up(implicit ev: Parent <:< JCompoundZipper[_]) = parent.replace(JObject(parent.here.fields.updated(fieldInParent, here))).asInstanceOf[Parent]
}
