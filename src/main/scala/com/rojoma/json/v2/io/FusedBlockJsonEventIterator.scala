...
class FusedBlockJsonEventIterator(input: Reader, fieldCache: FieldCache = IdentityFieldCache, blockSize: Int = 1024) extends AbstractBufferedIterator[JsonEvent] {
  ...
  private def badToken(expected: String): Nothing = {
    val pos = Position(nextCharRow, nextCharCol)
    val token = peekChar() match {
      case '{' => TokenOpenBrace()(pos)
      case '}' => TokenCloseBrace()(pos)
      case '[' => TokenOpenBracket()(pos)
      case ']' => TokenCloseBracket()(pos)
      case ',' => TokenComma()(pos)
      case ':' => TokenColon()(pos)
      case '"' | '\'' => TokenString(readString())(pos)
      case c if isDigit(c) || c == '-' => TokenNumber(readNumber())(pos)
      case c if Character.isUnicodeIdentifierStart(c) => TokenIdentifier(readIdentifier())(pos)
  ...
  private def readFieldNameEvent(expected: String): JsonEvent = {
    ...
    nonDatum(FieldEvent(fieldCache(field))(Position(row, col)))
  }
