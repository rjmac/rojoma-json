...
class JsonEventIteratorComparisonTests extends AnyFunSuite with Matchers with ScalaCheckPropertyChecks {
  ...
  def compare(a: Iterator[JsonEvent], b: Iterator[JsonEvent]): Unit = {
    while(true) {
