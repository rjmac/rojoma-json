...
rojoma-json is published for Scala version 2.10 and 2.11.  Versions
...
object Foo {
  implicit val jCodec = SimpleJsonCodecBuilder[Foo].build("a", _.a, "b", _.b)
}
```
...
them, or you will either get an exception when `build` is called
...
have `JsonCodec`s themselves, or be `Option`s wrapping around such
types.  `build` comes in variants that will handle up to 22 fields.

Beginning with version 2.2.0, rojoma-json also contains a macro-based
`AutomaticJsonCodecBuilder`.  Due to the experimental nature of macros
themselves, this is also a somewhat experimental feature.  It allows
fully automatic generation of `JsonCodec`s for classes which have
accessors that match the names and types of all non-implicit
parameters of the primary constructor.  The above example using this
...
The generation can be affected by placing annotations (also defined in
the `util` package) on the subject class's constructor's parameters.
The annotations are:
